...
| **Alberta Wildfire** (Government of Alberta) | Alberta agency fire records + current-season fire perimeters | Open Government Licence – Alberta (see the note below — the services themselves carry no embedded licence statement) | "Contains information licensed under the Open Government Licence – Alberta" |
...
- **Alberta Wildfire.** Keyless. The Alberta Wildfire Management Branch publishes the services behind its public status map on ArcGIS Online (`services.arcgis.com/Eb8P5h4CJk8utIBz`), `Query`-only, no token, and they answer `f=geojson` directly. The layer polls active fire points every 5 minutes and active perimeters every ~15 minutes (perimeters are surveyed products and move far slower than status text). Alberta publishes no documented rate limit for these services; the cadence is set by how fast the data changes. Alberta's fire season runs roughly March–October, so **zero active fires is a normal answer for months at a time, not an outage** — the layer then falls back to the year-to-date service, capped at 200 records, and flags every one of those rows as historical so a past fire is never presented as currently burning. This layer is not a substitute for NASA FIRMS and does not supersede it: FIRMS reports satellite thermal detections including unassessed ones, while these are agency records with a fire number, an assessed status, a cause and a response type. The two legitimately disagree, and both stay available.

  **Licence caveat worth knowing.** The ArcGIS items are shared publicly but carry **no `licenseInfo` or `accessInformation` field at all** — there is no licence statement attached to the services themselves. The Open Government Licence – Alberta is the Government of Alberta's standard licence for openly released information (every wildfire-related record on `open.alberta.ca` is published under it), so that is the attribution used here. That is an inference from Government of Alberta practice, not a declaration on the service. Anyone redistributing this data commercially should confirm the terms with Alberta Wildfire directly.

  The published geometry does not follow GeoJSON conventions and the converter compensates: the same perimeter has been served as both `Polygon` and `MultiPolygon` across requests, and a single `Polygon` can contain **disjoint** rings sharing one winding direction (the 2026-09-04 perimeter for GWF-058-2026 did exactly this, the second ring ~34x the area of the first). Rings are therefore classified by containment depth rather than by position or winding order. The records' epoch-millisecond date fields also disagree with their own Mountain Time strings by 12 hours, so `FIRE_STATUS_DATE` is parsed as America/Edmonton wall-clock and the epoch fields are not trusted for display.
